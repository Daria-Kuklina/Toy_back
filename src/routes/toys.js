...
router.post(
  ...
  auth("admin"),
  wrap(async (req, res) => {
    const { name, price, description, firm, producing_country, photo_id } = req.body;
...
router.get(
  ...
  wrap(async (req, res) => {
    ...
    const { id, name, price, description, firm, producing_country, photo_id } = await toysController.getToyById({ toyId });

    res.send({
      success: true,
      toy: { id, name, price, description, firm, producing_country, photo_id },
    });
  })
);

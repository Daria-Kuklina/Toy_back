...
exports.updateToy = async ({ toyId, name, price, description, firm, producing_country, photo_id }) => {
  try {
...
exports.getToyById = async ({ toyId }) => {
  const [record] = await knex("toys")
    .select("id", "name", "price", "description", "firm", "producing_country", "photo_id")
    .where({ id: toyId });
  if (!record) {
    throw new ControllerException("NOT_FOUND", "Toy has not been found");
  }
